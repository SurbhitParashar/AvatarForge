def generate_avatar(input_path, output_path):
        import os
        ...
        refined_path = input_path + "_refined.png"

        with open(refined_path, "wb") as output_file:
            output_file.write(output_data)


        # -------------------------------
        # ✅ STABLE FACE DETECTION (DeepFace)
        # -------------------------------
        ...
        # IMPORTANT: use original image
        image_path_refined = refined_path

        face = detect_face_and_crop(image_path_refined)
        ...
        forehead_top = 0
        forehead_bottom = int(h * 0.25)
        forehead_left = int(w * 0.25)
        forehead_right = int(w * 0.75)
        ...
        dominant_hair_color = kmeans.cluster_centers_[0]
        dominant_hair_color = [int(c) for c in dominant_hair_color]
        ...
        def detect_beard_level(image_path_refined):
            ...
            beard_density = np.mean(edges)

            if beard_density < 10:
                return "DEFAULT"
            elif beard_density < 20:
                return "BEARD_LIGHT"
            elif beard_density < 30:
                return "BEARD_MEDIUM"
            else:
                return "BEARD_MAJESTIC"
